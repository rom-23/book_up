...
public static function extractIds($array, $indicateur, $clear = true) {
        $i = 0;
        $array_id = array();
...
function convert_csv_file($file){// recupere le csv du jour GetQuanty
...
public static function getYears($max) {
        $years = array();
...
public static function array_key_last( $ll ) {
        $key = NULL;
...
public static function convertDate($date){
    ...
    public static function resolve($value, $if, $def, $value_to_print = null) {
        if ($value === $if) {
            return $def;
        } else {
    ...
    public static function format_phone($telephone_brut, $fullName) {
    ...
    public static function format_mail($mail, $fullName) {
        if ($mail == 'NC') {
            return $mail;
        } else {
    ...
    public static function format_site_web($site_web) {
        if ($site_web == 'NC') {
            return $site_web;
        } else {
    ...
    public static function format_date_fr_toStr($str_date) {
        $date = date_create($str_date);
        return date_format($date, 'd/m/Y');
    }

    public static function format_date_us_toStr($str_date) {
        $date = date_create($str_date);
        return date_format($date, 'Y/m/d');
    }


    public static function showPre($value) {
        echo '<pre>';
        if(is_array($value)) {
            print_r($value);
        }
    ...
    /**
     * Encode une array ï¿½ une dimenssion en utf-8
     * et encode les retours ï¿½ la ligne
     *
     * @param $array array - L'array ï¿½ traiter
     * @param $encode_utf-8 boolean - True par dï¿½faut : true si on dï¿½sire encoder en UTF-8 (true pour mssql et false pour mysql)
     * @param $addslashes boolean - True par dï¿½faut : true pour ajoute les slashs pour les quotes (true pour mssql et false pour mysql)
     * @return array - L'array encodï¿½
     */
    public static function encode_array($array, $encode_utf8 = true, $addslashes = false) {
